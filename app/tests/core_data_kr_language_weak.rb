...
class FAIRTest
  ...
  def self.core_data_kr_language_weak(guid:)
    FAIRChampion::Output.clear_comments

    output = FAIRChampion::Output.new(
      testedGUID: guid,
      meta: core_data_kr_language_weak_meta
    )
    output.comments << "INFO: TEST VERSION '#{core_data_kr_language_weak_meta[:testversion]}'\n"
  ...
  def self.testIdentifier(guid:, output:)
    ...
    if headers
      if headers.keys.include?(:content_type)
        ...
        structuredformats = Utils::RDF_FORMATS.values.append Utils::XML_FORMATS.values.append Utils::JSON_FORMATS.values.append [
          'text/xhtml+xml', 'application/xhtml+xml'
        ]
        structuredformats.flatten!
        if structuredformats.include?(type)
  ...
  def self.core_data_kr_language_weak_api
    api = OpenAPI.new(meta: core_data_kr_language_weak_meta)
    api.get_api
  end

  def self.core_data_kr_language_weak_about
    dcat = ChampionDCAT::DCAT_Record.new(meta: core_data_kr_language_weak_meta)
    dcat.get_dcat
  end
end
