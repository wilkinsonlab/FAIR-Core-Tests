...
class FAIRTest
  def self.core_metadata_protocol_meta
    {
  ...
  def self.core_metadata_protocol(guid:)
    FAIRChampion::Output.clear_comments

    output = FAIRChampion::Output.new(
      testedGUID: guid,
      meta: core_metadata_protocol_meta
    )

    output.comments << "INFO: TEST VERSION '#{core_metadata_protocol_meta[:testversion]}'\n"
  ...
  def self.core_metadata_protocol_api
    api = OpenAPI.new(meta: core_metadata_protocol_meta)
    api.get_api
  end

  def self.core_metadata_protocol_about
    dcat = ChampionDCAT::DCAT_Record.new(meta: core_metadata_protocol_meta)
    dcat.get_dcat
  end
end
