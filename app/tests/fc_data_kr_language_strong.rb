class FAIRTest
  ...
  def self.fc_data_kr_language_strong(guid:)
    ...
    output = FtrRuby::Output.new(
      testedGUID: guid,
      meta: fc_data_kr_language_strong_meta
    )
    output.comments << "INFO: TEST VERSION '#{fc_data_kr_language_strong_meta[:testversion]}'\n"
  ...
  def self.fc_data_kr_language_strong_api
    api = FtrRuby::OpenAPI.new(meta: fc_data_kr_language_strong_meta)
    api.get_api
  end

  def self.fc_data_kr_language_strong_about
    dcat = FtrRuby::DCAT_Record.new(meta: fc_data_kr_language_strong_meta)
    dcat.get_dcat
  end
end
